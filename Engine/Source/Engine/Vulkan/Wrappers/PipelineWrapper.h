...
public:
    PipelineWrapper(allocators::StackAllocator<void>& allocator, BasicRenderer* renderer, DeviceWrapper* device);
    ~PipelineWrapper();
    ...
    static VkShaderModule create_shader_module(VkDevice device, const allocators::stack_vec<char>& shader_code);
};
