...
VulkanWrapper::VulkanWrapper() : window_(1000, 800, "Vulkan Window"),
    ...
    m_swap_chain_(std::make_unique<SwapChain>(window_.raw_window(), m_surface_, &m_device_)),
    m_pipeline_(m_swap_chain_.get(), &m_device_),
    m_command_buffer_(&window_, &m_device_, &m_pipeline_, m_swap_chain_.get(), m_device_.get_graphics_queue_family().graphics_family_index) {
...
void VulkanWrapper::draw_frame() {
    ...
    if (m_command_buffer_.present_command_buffer(m_current_frame_, image_index) || window_.was_resized()) {
        std::cerr << "Window resized. Moving on...\n";
        window_.toggle_resized();
        reset_swap_chain();
    }
...
void VulkanWrapper::reset_swap_chain() {
    m_swap_chain_.reset();
    m_swap_chain_ = std::make_unique<SwapChain>(window_.raw_window(), m_surface_, &m_device_);
    m_pipeline_.set_new_swap_chain_ptr(m_swap_chain_.get());
    m_command_buffer_.reset_swap_chain_ptr(m_swap_chain_.get());
}
